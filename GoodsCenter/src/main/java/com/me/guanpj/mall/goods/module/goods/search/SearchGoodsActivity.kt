package com.me.guanpj.mall.goods.module.goods.search
...
class SearchGoodsActivity : BaseActivity(), View.OnClickListener {
    ...
    private fun loadData() {
        val set = AppPrefsUtils.instant.getStringSet(GoodsConstant.SP_SEARCH_HISTORY)
    ...
    override fun onClick(v: View) {
        when (v.id) {
            ...
            //清除历史记录
            R.id.mClearBtn -> {
                AppPrefsUtils.instant.remove(GoodsConstant.SP_SEARCH_HISTORY)
                loadData()
            }
        }
    }
    ...
    private fun doSearch() {
        if (mKeywordEt.text.isNullOrEmpty()) {
            ...
            AppPrefsUtils.instant.putStringSet(GoodsConstant.SP_SEARCH_HISTORY, mutableSetOf(inputValue))
